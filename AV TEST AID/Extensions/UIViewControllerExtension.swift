...
extension UIViewController {
    ...
    @objc
    func showDialog(withMessage message: String, andTitle title: String? = nil, action: UIAlertAction? = nil) {

        let alertController = UIAlertController(title: title, message: message, preferredStyle: UIAlertController.Style.alert)

        let action = action ?? UIAlertAction(title: "OK", style: .default) { _ in
        }
